...
import sys
sys.path.append(('../'))
from util import read_image, create_gradient_map, im2tensor01_cpu, create_probability_map, nn_interpolation

'''
# ------------------------------------------
# data generator of KernelGAN-FKP
# ------------------------------------------
'''

class DataGenerator_FKP(Dataset):
    """
    ...
    def __init__(self, conf, gan):
        # Default shapes
        self.g_input_shape = conf.input_crop_size # 64
        self.d_input_shape = gan.G.output_size  # shape entering D downscaled by G, 26
        self.d_output_shape = gan.D.output_size # self.d_input_shape - gan.D.forward_shave # 26-6=20
    ...
    def next_crop(self, for_g, idx):
        ...
        return im2tensor01_cpu(crop_im) # transform to tensor, [C,H,W]

    def make_list_of_crop_indices(self, conf):
        sampling = conf.max_sampling
        prob_map_big, prob_map_sml = self.create_prob_maps(scale_factor=conf.scale_factor)
        crop_indices_for_g = np.random.choice(a=len(prob_map_sml), size=sampling, p=prob_map_sml)
        crop_indices_for_d = np.random.choice(a=len(prob_map_big), size=sampling, p=prob_map_big)
