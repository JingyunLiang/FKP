...
'''
# ------------------------------------------------
# util.py for original KernelGAN and KernelGAN-KP
# ------------------------------------------------
'''
...
def post_process_k_crop2(k, n):
    ''' Crop the estimated kernel from 13x13 to 11x11 and post process the kernel and '''
    k = k[:-2, :-2]
    return post_process_k(k, n, sf=2)
...
def save_final_kernel_png(k_2, conf, gt_kernel, step=''):
    ...
    if conf.X4:
        ...
        sio.savemat(savepath_mat, {'Kernel': k_4})
        plot_kernel(gt_kernel, k_4, savepath_png)
    else:
        sio.savemat(savepath_mat, {'Kernel': k_2})
        plot_kernel(gt_kernel, k_2, savepath_png)
...
def evaluation_dataset(input_dir, conf, used_iter=''):
    ...
    for filename in filesource:
        ...
        if conf.SR:
            ...
            crop_border = 4 if conf.X4 else 2
