...
# It is a parameter-free module  ^_^
...
#        util.imsave(util.tensor2uint(x), 'test_57_gan_0.png')
...
#            util.imsave(util.tensor2uint(x), 'test_57_gan_z'+str(i+1)+'.png')

            x = self.p(torch.cat((x, ab[:, i+self.n:i+self.n+1, ...].repeat(1, 1, x.size(2), x.size(3))), dim=1))

#            util.imsave(util.tensor2uint(x), 'test_57_gan_x'+str(i+1)+'.png')
